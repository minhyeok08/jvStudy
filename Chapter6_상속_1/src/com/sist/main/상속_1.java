 ...
 * 		= 다른 데이터형	: 클래스 (*****)
 * 	3. 클래스
 * 		= 클래스 정의 방법 (객체지향 프로그램 => 한개의 클래스로 만드는 것이 아니라 클래스와 클래스의 연관관계 => Spring(DI))
 * 						행위자 (웹: 사용자, 관리자)
 * 		*** 접근지정어 (권한)
 ...
 * 				1. 클래스명과 동일
 * 				2. 멤버변수에 대한 초기화
 * 				3. 생성자가 없는 경우에는 자도응로 컴파일러가 기본생성자를 추가
 * 												 -------
 * 												 매개변수가 없는 생성자
 * 				예)
 * 					class A
 ...
 * 						class A
 * 						{
 * 							// A(){} 추가되지 않는다 // 생성자가 3개 존재하기 때문에 기본생성자 자동생성X
 * 							A(int a){} ==> 생성자
 * 							A(double d){} ==> 생성자
 * 							A(int a, int b){} ==> 생성자
 * 						}
 ...
 * 				this사용이 가능
 * 			}
 * 			static void aaa() ==> 소속이 아니라 공유하는 메소드
 * 			{
 * 				this가 존재하지 않는다
 ...
 * 		A a = new A(); => this // 자신의 객체주소를 갖고있는애가 this
 * 		A b = new A(); => this
 * 		-----------------
 ...
 * 		  }
 * 
 * 		= 기존에 만들어진 클래스를 확장해서 새로운 클래스 만들어서 사용
 * 			예)
 * 				class 게시판
 * 				{
 * 					글쓰기
