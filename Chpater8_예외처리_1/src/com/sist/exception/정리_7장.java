 ...
 * 			class A
 * 			class B extends A => A가 가지고 있는 모든 내용을 갖고 온다
 ...
 * 							  ----------------
 * 							  메모리 할당 시에 상속을 받은 클래스를 이용해서 메모리 할당을 한다
 * 							  미완성 클래스(메소드가 구현이 안된 상태)
 * 										------------------
 * 							  => 설계 , 같은 기능 (처리를 프로그램 맞게 할 때)
 * 									   버튼 클릭 , 마우스 클릭...
 * 										게시판 : 글쓰기
 * 				 B d = (B)c; => 명시적 형변환 (강제)
 * 				------------ 라이브러리에서 많이 사용
 ...
 * 
 * 			class A
 * 			class B extends extends A
 * 			class D extends extends B
 * 			class E extends extends B
 * 			-------------------------
 * 			A a = new A()
 * 			A a = new B()
 * 			A b = new B()
 ...
 *		  ** 클래스(단일) / 인터페이스(다중)
 *		--- 설계도(구현이 안된 상태) => 미완성 클래스 (자신이 메모리 할등을 할 수 없다)
 ...
 *		-----------------------------------------------	
 *		멤버변수	인스턴스변수		상수형변수만 사용(생성자 X)
 *				정적변수
 *		-----------------------------------------------
 *		메소드	구현된 메소드		구현이 안된 메소드만
 *				구현이 안된 메소드
 *		-----------------------------------------------	
 *		기법		오버라이딩			오버라이딩
 *		-----------------------------------------------
 ...
 *				=> public static final int a =  10; (컴파일러는 이렇게 인식)
 *				   ------------------- 없으면 자동 추가
 *				---------------------
 *				구현이 안된 메소드 목록
 *				void display();
 ...
 *				구현된 메소드 목록
 *				default void aaa(){}
 *					=> public default void aaa(){} 로 인식	
 *				static void bbb(){}
 *					=> public default void bbb(){} 로 인식
 *				---------------------
 *				}
 *				==> 인터페이스는 변수나 메소드가 public (연결 목적이기 떄문에 => 언제든 연결이 가능)
 *		-----------------------------------------------------
 *		
 *		상속
 *				 extends
 ...
 *		------------
 *		Stack			|	메모리 자체 관리
 *		지역변수 / 매개변수	|	=> {} 종료되면 자동 소멸
 *		------------
 *		Heap 영역						| 1. 사용이 없는 경우
 *		객체 / 배열 / 인터페이스 / 열거형	| 2. null인 경우
 *		------------
 *		==> 전체 관리 : JVM
 *
 */
